...
The helper waits for supported video before starting sync, serializes commands, checks again after detected display-mode changes, and stops sync on sleep. It runs locally on the TV through **tvQuickActions + HTTP Shortcuts**. No AI service or computer is needed during normal operation.

**Full source code is here.** The Android helper is 558 lines of Java across eight files, including optional room scenes. It must be built into an Android app; these Java files cannot be pasted into HTTP Shortcuts. The core HTTP requests themselves are only status, start and stop.
...
The public package has offline/build checks. Sleep/wake, reboot and several media apps were tested on the earlier original installation; this does not establish compatibility with every device or make every media app emit useful scene events. See the testing guide for exact boundaries.
